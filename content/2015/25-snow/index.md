...
Like with the previous day, I'm writing and coding simultaneously. 

I skipped reading the input data. That's just two numbers, so I put them in the code. At this point I don't need to practise parsing simple text files, right? I also added the other constants from the instruction:
...
But computers are really fast at simple arithmetic, so let's just iterate until we find one. We're starting at the first number which is at row 0, column 0. I'll also keep track of maximum row and column reached so far and I'll use the full loop. Perhaps later I'll be able to simplify. Let's try first 10 codes to begin.
...
But I'm solving it in May. So let's rewrite the code to C. It's only a simple loop and arithmetic operations, shouldn't be hard. One gotcha - when we multiply the code, we get a number too big for a standard **int**, but **unsigned long long** works just fine.
...
That's over 50 000 times faster! 
